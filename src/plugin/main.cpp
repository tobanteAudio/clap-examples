...
static const clap_plugin_descriptor_t s_my_plug_desc = {
    ...
    .features = (const char **)(features.data()),
};
...
static uint32_t my_plug_audio_ports_count(const clap_plugin_t *plugin,
                                          bool is_input) {
  return 1;
}

static bool my_plug_audio_ports_get(const clap_plugin_t *plugin, uint32_t index,
                                    bool is_input,
...
static uint32_t my_plug_note_ports_count(const clap_plugin_t *plugin,
                                         bool is_input) {
  return 1;
}

static bool my_plug_note_ports_get(const clap_plugin_t *plugin, uint32_t index,
...
static bool my_plug_init(const struct clap_plugin *plugin) {
  ...
  plug->hostThreadCheck = (clap_host_thread_check_t *)plug->host->get_extension(
      plug->host, CLAP_EXT_THREAD_CHECK);
  plug->hostLatency = (clap_host_latency_t *)plug->host->get_extension(
...
static void my_plug_process_event(my_plug_t *plug,
...
static const void *my_plug_get_extension(const struct clap_plugin *plugin,
                                         const char *id) {
