...
static const char *_features[] = {"", NULL};

static clap_plugin_descriptor _descriptor = {
    CLAP_VERSION,        "com.cockos.clap-example-0",
    "CLAP Volume/Pan",   "cockos",
    "https://reaper.fm", "https://reaper.fm",
    "https://reaper.fm", "0.0.1",
    "volume and pan",    _features,
};
...
struct Example_0 : public Plugin {
  ...
  clap_process_status _process(const clap_process *process, int num_channels,
    ...
    for (int c = 0; c < num_channels; ++c) {
      ...
      for (int i = start_frame; i < end_frame; ++i) {
        cout[i] = cin[i] * adj;
        if (cin[i] > m_peak_in[c])
          m_peak_in[c] = cin[i];
        if (cout[i] > m_peak_out[c])
          m_peak_out[c] = cout[i];
        adj += d_adj;
      }
    }
  ...
  clap_process_status process(const clap_process *ctx) {
    ...
    for (int c = 0; c < 2; ++c) {
      m_peak_in[c] *= decay;
      m_peak_out[c] *= decay;
      if (m_peak_in[c] < 1.0e-6)
        m_peak_in[c] = 0.0;
      if (m_peak_out[c] < 1.0e-6)
        m_peak_out[c] = 0.0;
    }
  ...
  void flushParameter(const clap_input_events *in,
    ...
    for (int i = 0; i < in->size(in); ++i) {
      ...
      if (!evt || evt->space_id != CLAP_CORE_EVENT_SPACE_ID)
        continue;
      if (evt->type == CLAP_EVENT_PARAM_VALUE) {
        const clap_event_param_value *pevt =
            (const clap_event_param_value *)evt;
...
clap_plugin_descriptor *plugin_impl__get_descriptor_0() { return &_descriptor; }
