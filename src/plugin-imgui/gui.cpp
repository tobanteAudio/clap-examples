...
bool Plugin::gui__get_size(uint32_t *width, uint32_t *height) {
  if (m_w > 0 && m_h > 0) {
    ...
    *height = m_h;
    ...
    plugin_impl__get_preferred_size(width, height);
  return true;
}
...
bool Plugin::gui__set_transient(const clap_window *window) { return false; }

void Plugin::gui__suggest_title(const char *title) {}

bool Plugin::gui__show() { return true; }

bool Plugin::gui__hide() { return true; }